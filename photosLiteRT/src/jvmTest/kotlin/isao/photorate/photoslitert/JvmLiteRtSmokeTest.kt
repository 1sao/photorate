...
/**
 * Real-inference smoke test: the full LiteRT hand pipeline runs on the JVM via
 * kmplitert (its JVM jar bundles the LiteRT dylib) over the converted models in
 * ml/litert/converted and the sample photos in plans/samples. This proves the
 * shared pipeline + JVM engine actually execute — the same code path the iOS
 * and Android factories use — instead of just type-checking.
 */
class JvmLiteRtSmokeTest {

    // jvmTest's working dir is the module dir; the models/samples live at root.
    ...
    fun thumbRatingRunsRealInference() {
        ...
        // Same tuned gates the app uses on-device (LandmarkModel.LITERT).
        val landmarker = factory.createFromOptions(LandmarkModel.LITERT.defaultOptions)
        try {
            // A known thumbs-up sample (5/ = score 5 in the app's dataset).
            ...
            assertTrue(detected.hands.isNotEmpty(), "no hand detected on $sample")
