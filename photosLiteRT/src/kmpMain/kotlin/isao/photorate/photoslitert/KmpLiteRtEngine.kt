...
/**
 * kmplitert-backed [LiteRtEngine] for the iOS + JVM targets. The kmplitert API is suspend-based
 * (`LiteRTCompiler`), so this adapter bridges it to the synchronous pipeline with [runBlocking] —
 * inference itself is blocking C calls either way.
 *
 * The input/output buffer lists are fetched once at init and reused for every run, mirroring the
 * Android CompiledModel engine's pre-allocated buffers.
 */
class KmpLiteRtEngine
private constructor(
...
internal fun LiteRtAccelerator.toKmp(): io.github.leitingzi.kmplitert.core.LiteRTAccelerator =
  when (this) {
...
actual fun createLiteRtEngine(source: ModelSource, config: EngineConfig): LiteRtEngine {
  ...
  return runBlocking {
    KmpLiteRtEngine.create(
      file.path,
      config.accelerator.toKmp(),
    )
  }
}
