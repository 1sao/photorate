...
/**
 * iOS [HandLandmarkerFactory] backed by the shared LiteRT pipeline and
 * kmplitert engines. Models ship as bundle resources; the app passes the
 * directory containing the converted tflite files.
 */
class IosLiteRtHandLandmarkerFactory(private val modelsDir: String) : HandLandmarkerFactory {

    override fun createFromOptions(options: HandLandmarkerOptions): HandLandmarker = LiteRtHandLandmarker(
...
class IosLiteRtAppClipSearchFactory(private val modelsDir: String, private val tokenizerJson: String) : AppClipSearchFactory {

    override fun createFromOptions(options: AppClipSearchFactory.Options): AppClipSearch = LiteRtAppClipSearch(
        engine = createLiteRtEngine(
