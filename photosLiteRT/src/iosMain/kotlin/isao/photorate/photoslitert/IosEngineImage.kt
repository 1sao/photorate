...
/**
 * iOS [EngineImage] backed by a [UIImage]. Pixels are extracted once, into an
 * ARGB [IntArray], and the pure-Kotlin [PixelEngineImage] ops do the rest.
 */
class IosEngineImage private constructor(
    ...
    override fun resized(w: Int, h: Int): EngineImage = PixelEngineImage(width, height, pixels).resized(w, h)

    override fun drawnOn(w: Int, h: Int, dx: Int, dy: Int, background: Int): EngineImage =
        PixelEngineImage(width, height, pixels).drawnOn(w, h, dx, dy, background)

    override fun cropped(x0: Int, y0: Int, w: Int, h: Int): EngineImage? = PixelEngineImage(width, height, pixels).cropped(x0, y0, w, h)

    override fun rotatedWithoutCrop(degrees: Float): EngineImage = PixelEngineImage(width, height, pixels).rotatedWithoutCrop(degrees)

    override fun scaledAboutCenter(scale: Float, targetSize: Int, background: Int): EngineImage =
        PixelEngineImage(width, height, pixels).scaledAboutCenter(scale, targetSize, background)
    ...
    companion object {
        ...
        fun fromImage(image: UIImage): IosEngineImage {
            ...
            // Draw + read INSIDE usePinned: CGContextDrawImage writes through the
            // pinned buffer's address, which is only valid while pinned.
            return byteArray.usePinned { pinned ->
                ...
                CGContextDrawImage(context, CGRectMake(0.0, 0.0, width.toDouble(), height.toDouble()), image.CGImage)
                ...
                IosEngineImage(image, width, height, pixels)
            }
        }
    }
}
