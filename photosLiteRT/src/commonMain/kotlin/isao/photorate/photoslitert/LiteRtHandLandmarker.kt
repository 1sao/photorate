...
/**
 * The shared LiteRT [HandLandmarker] pipeline: RTMDet hand detection -> multi-rotation RTMPose
 * search -> gesture rating. This is a port of the verified ONNX pipeline
 * (`photosOnnx.AndroidOnnxHandLandmarker`) that swaps ONNX Runtime sessions for the LiteRT
 * conversions of the same two models (ml/litert/ recipe; assets shipped from ml/litert/converted):
 *
 ...
 *
 * Platform-neutral: images come in as [EngineImage] and inference goes out through the injected
 * [LiteRtEngine]s. Android wires CompiledModel engines (GPU-first with CPU fallback), iOS/JVM
 * kmplitert engines. The scan runs on background threads with no EGL context; OpenCL buffers need
 * none (only the GL backend's textures would).
 */
class LiteRtHandLandmarker(
  ...
  private val detectorTensor =
    ImageTensor(
      ...
      // mmdet normalization
      // (123.675/58.395 ...) ==
      // ImageNet mean/std scaled
      // to 0..1; ImageTensor divides
      // pixels by 255 first, so the
      // equivalence
      // is exact (verified against
      // the ONNX pipeline's
      // bitmapToCHWMeanStd).
  ...
  override fun detect(candidate: LandmarkCandidate): LandmarkedImage {
    ...
    return LandmarkedImage(
      hands = timed.value,
      detectedInMs = timed.duration.inWholeMilliseconds,
    )
  }
  ...
  private fun runPipeline(image: EngineImage): List<LandmarkedImage.Hand> {
    val boxes = detectBoxes(image) // pixel coords in the
    // original image
    ...
    // Edge fallback: a hand mostly out of frame at the
    // left/right image
    // edge is invisible to full-frame RTMDet (5_kimbo —
    // the thumb is in
    // shot, the rest off-frame). Probe the edge strips
    // for a confident
    // thumb chain and rate it as a low-certainty THUMBS
    // guess
    // (edgeDetected hands take the classifier's
    // thumb-only path).
    if (hands.isEmpty()) {
  ...
  private fun detectBoxes(image: EngineImage): List<Pair<IntArray, Float>> {
    ...
    val ratio =
      min(
        DETECTOR_SIZE / iw.toFloat(),
        DETECTOR_SIZE / ih.toFloat(),
      )
    val nw = max((iw * ratio).toInt(), 1)
    val nh = max((ih * ratio).toInt(), 1)
    // Top-left letterbox: resize to (nw, nh) then place
    // at (0, 0) on a
    // 320x320 canvas (transparent black == the old Bitmap
    // default).
    val padded =
      image
        .resized(nw, nh)
    ...
    // Raw anchors boxes [1,2100,4] + scores [1,2100,1] in
    // 320-space
    // (top-left padded). Post-processing (score filter +
    // NMS) uses the
    ...
    for (i in keep) {
      ...
      val bx0 =
        max(
          (boxesRaw[o] / ratio).toInt(),
          0,
        )
      val by0 =
        max(
          (boxesRaw[o + 1] / ratio).toInt(),
          0,
        )
      val bx1 =
        min(
          (boxesRaw[o + 2] / ratio).toInt(),
          iw,
        )
      val by1 =
        min(
          (boxesRaw[o + 3] / ratio).toInt(),
          ih,
        )
   ...
   * undistorted regardless of image aspect ratio. Storage code un-rotates via
   * [LandmarkedImage.Hand.rotationDegrees] and normalizes to 0..1.
   */
  private fun rtmposeRating(
    image: EngineImage,
    box: IntArray,
    ...
    // Highest-confidence hand across ALL rotations,
    // including those that
    // form no gesture: the dev-mode best-guess rater
    // still rates those, so
    ...
    search@ for (factor in FALLBACK_FACTORS) {
      ...
      for (deg in candidates) {
        ...
        val result =
          HandWithScore(
            hand,
            HandGestureClassifier.classify(hand),
            kpMean,
          )
    ...
    // Prefer the image-space rating, but only when
    // RTMPose is reasonably
    // confident in those keypoints; a low-confidence
    // upright crop can mis-
    // rate, and the rotated search then has better data
    // to work with. When
    // no rotation forms a gesture at all, fall back to
    // the highest-
    // confidence hand (the dev-mode rater can still guess
    // from it).
    val winner =
      when {
        ...
        // pick
        // the
        // winner that will fail the confidence check
    ...
    // Confidence tiers replace the sparse presence gate.
    // Below the floor
    // the box is not a hand at all (no_score stays
    // filtered); ROCK/OK need
    // higher confidence than THUMBS (a curled-finger read
    // at low kp is
    // usually a holding/gripping false positive). Hands
    // with no recognized
    // gesture use the non-THUMBS gate (the dev-mode rater
    // marks its guess
   ...
   */
  private fun edgeThumbFallback(
    image: EngineImage,
    fullFrameBest: Float,
  ): List<LandmarkedImage.Hand> {
    // The full-frame detector already ran for the main
    // pass; an edge hand
    // still leaves a weak full-frame response (5_kimbo:
    // 0.167), so below
    // the hint threshold the image has no hand signal at
    // all — skip.
  ...
  private fun edgeThumbScan(
    ...
    for (deg in CANDIDATE_ROTATIONS) {
      ...
      // Prefer the most confident
      // thumb chain, then kp mean
      // (mirrors the
      // Python edge_thumb_scan
      // selection).
      if (
        best == null ||
      ...
      // Deg-0-first acceptance: the
      // upright read is the only
      // gate-qualifying rotation on
      // the edge sample (5_kimbo —
      // deg 0
      // passes the thumb-chain gates
      // at angle 22.4°, 90/180/270
      // fail),
      // so accept it immediately and
      // skip the rotated sweep. The
      // sweep
      // still runs when deg 0 fails
      // the gates (a sideways thumb
      // can
      // still qualify rotated).
      // Prunes 4 RTMPose runs to 1
      // per strip.
      if (deg == 0f) break
    }
   ...
   * with RGB mean/std (matches the verified Python). The converted tflite outputs raw heatmaps
   * simcc_x/simcc_y [1, 21, 512]; decode = argmax / simcc_split_ratio (2.0) mapped back through the
   * inverse affine to crop pixels, with min(peak_x, peak_y) as confidence
  ...
  private fun rtmposeLandmarks(crop: EngineImage): FloatArray? {
    ...
    // Inverse of the top-down affine (see topDownAffine):
    // undo the scale
    // about the crop center to map decoded 256-space bins
    // to crop pixels.
    val bboxW = 1.25f * w
    val bboxH = 1.25f * h
    ...
    for (i in 0 until NUM_LANDMARKS) {
      ...
      // Confidence: min of the two
      // heatmap peaks (the baked
      // decode's
  ...
  private fun rotateAndCropRectangle(
    ...
    val rw = width.toInt()
    val rh = height.toInt()
    // cropRect expects the CENTER (it subtracts half the
    // size internally), so
    // pass ccx/ccy directly — passing ccx - rw/2
    // double-subtracted and made
    // every tall hand crop out-of-bounds (y0 < 0),
    // killing all detections.
    val final =
      cropRect(
        rotated,
        ccx,
  ...
  private fun boundingBoxFromRotatedRect(
    ...
    val hw = w / 2f
    val hh = h / 2f
    // Four corners of the rotated rect (cv2.boxPoints
    // equivalent).
  ...
  private fun imageRotationWithoutCrop(
    image: EngineImage,
    degree: Float,
  ): EngineImage = image.rotatedWithoutCrop(degree)
  ...
  private companion object {
    // Minimum mean RTMPose keypoint confidence for the
    // deg-0 (image-space)
    // rating to be trusted over the rotated search's best
    // rating.
    const val MIN_RTMPOSE_KP = 0.3f
    const val DETECTOR_SIZE = 320
    const val RTMPOSE_SIZE = 256

    // Detected boxes are expanded 1.2x around the center
    // and made square so
    // the crops include the whole hand (mirrors the
    // verified Python
    // pipeline). The search additionally tries a 0.85x
    // (tighter) crop so
    // tall/thin detection boxes don't drown the hand in
    // background.
    const val BOX_EXPANSION = 1.2f
    val FALLBACK_FACTORS = floatArrayOf(1.2f, 0.85f)

    // Edge thumb-only fallback: a single half-width strip
    // per edge over
    // the lower 60% of the image (tuned so 5_kimbo's
    // left-edge hand is
    // found at det 0.46, thumbC 0.66, while no_score
    // strips fire
    // nothing). Tradeoff: an edge hand in the top 40%
    // would be missed
    // (re-add bands if a sample needs it).
    const val EDGE_STRIP_FRACTION = 0.5f
    val EDGE_BANDS = listOf(0.4f to 1f)

    // Skip the fallback when the full-frame detector's
    // best box is below
    // this hint (5_kimbo leaves a 0.167 full-frame
    // response). Kept at
    // 0.10, not higher: the zoomed strip detection is
    // largely independent
    // of the full-frame response, so a hand even more out
    // of frame could
    // score < 0.12 full-frame yet still read a confident
    // thumb in an edge
    // strip. 0.10 still skips genuine no-hand photos
    // (their detector
    // noise is far lower), making them cost 0 strip
    // detections.
    const val EDGE_HINT_DET = 0.10f
    val CANDIDATE_ROTATIONS = listOf(0f, 90f, 180f, 270f)

    // Minimum side length (px) for a box to be worth
    // landmarking.
