...
/**
 * Pure-Kotlin [EngineImage] backed by an ARGB [IntArray]. Serves the iOS and
 * JVM targets (Android wraps `Bitmap` instead so the verified on-device
 * pipeline keeps Bitmap/Canvas semantics exactly).
 *
 * Ops implement the same math as the original Android Bitmap pipeline:
 * bilinear resize, inverse-mapped rotation/affine, canvas draw.
 */
class PixelEngineImage(override val width: Int, override val height: Int, private val argb: IntArray) : EngineImage {
    ...
    override fun resized(w: Int, h: Int): EngineImage {
        ...
        for (y in 0 until h) {
            ...
            val y0 = floor(srcY).toInt().coerceIn(0, height - 1)
            val y1 = (y0 + 1).coerceIn(0, height - 1)
            val fy = srcY - y0
            for (x in 0 until w) {
                val srcX = (x + 0.5f) * scaleX - 0.5f
                val x0 = floor(srcX).toInt().coerceIn(0, width - 1)
                val x1 = (x0 + 1).coerceIn(0, width - 1)
                val fx = srcX - x0
                out[y * w + x] = bilinear(x0, y0, x1, y1, fx, fy)
            }
        }
    ...
    override fun cropped(x0: Int, y0: Int, w: Int, h: Int): EngineImage? {
        ...
        for (y in 0 until h) {
            ...
            argb.copyInto(out, y * w, srcStart, srcStart + w)
        }
        return PixelEngineImage(w, h, out)
    }

    /**
     * Inverse-mapped rotation about the center with canvas expansion. The
     * matrix matches `Matrix.postRotate(-deg, w/2, h/2)` +
     * `postTranslate(boundW/2 - w/2, boundH/2 - h/2)`; the pipeline only ever
     * passes multiples of 90°, where bilinear sampling is exact.
     */
    override fun rotatedWithoutCrop(degrees: Float): EngineImage {
        ...
        for (y in 0 until boundH) {
            for (x in 0 until boundW) {
                ...
                out[y * boundW + x] = sample(srcX, srcY)
            }
        }
    ...
    override fun scaledAboutCenter(scale: Float, targetSize: Int, background: Int): EngineImage {
        ...
        for (y in 0 until targetSize) {
            for (x in 0 until targetSize) {
                ...
                out[y * targetSize + x] = sample(srcX, srcY)
            }
        }
    ...
    private fun bilinear(x0: Int, y0: Int, x1: Int, y1: Int, fx: Float, fy: Float): Int {
        val p00 = pixel(x0, y0)
        val p10 = pixel(x1, y0)
        val p01 = pixel(x0, y1)
        val p11 = pixel(x1, y1)
        ...
        for (shift in CHANNEL_SHIFTS) {
            val c = (p00 shr shift and 0xFF) * a00 + (p10 shr shift and 0xFF) * a10 +
                (p01 shr shift and 0xFF) * a01 + (p11 shr shift and 0xFF) * a11
    ...
    private companion object {
        // ARGB channel byte offsets for [bilinear] (blue=0, green=8, red=16,
        // alpha=24). Hoisted out of the per-pixel loop to avoid an allocation.
    ...
    private fun sample(srcX: Float, srcY: Float): Int {
        ...
        val x1 = x0 + 1
        val y1 = y0 + 1
        if (x0 !in 0 until width || y0 !in 0 until height || x1 !in 0 until width || y1 !in 0 until height) return 0
