...
/**
 * The only per-platform seam in this module. The whole vision pipeline (detection, multi-rotation
 * RTMPose search, gesture rating, MobileCLIP embedding) is shared and drives engines through this
 * interface; each target supplies its own [createLiteRtEngine] implementation:
 *
...
/**
 * Creates a compiled engine for [source] with the platform's native LiteRT runtime. Android:
 * `CompiledModel` via [ModelSource.Asset]. iOS/JVM: kmplitert `LiteRTCompiler` via
 * [ModelSource.File].
 */
