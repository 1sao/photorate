...
/**
 * The shared LiteRT [AppClipSearch]: the single-graph MobileCLIP-S1 model
 * (vision + text encoders in one tflite, see ml/litert/converted/
 * CLIP_S1_README.md) plus the CLIP tokenizer.
 *
 ...
 */
class LiteRtAppClipSearch(private val engine: LiteRtEngine, private val options: AppClipSearchFactory.Options, tokenizerJson: String) :
    AppClipSearch {
    ...
    override fun embedText(text: String): FloatArray {
        ...
        engine.writeLongInput(1, LongArray(ids.size) { ids[it].toLong() })
        engine.run()
    ...
    private fun preprocess(image: EngineImage): FloatArray {
        ...
        val cropped = resized.cropped(left, top, IMAGE_SIZE, IMAGE_SIZE) ?: resized
