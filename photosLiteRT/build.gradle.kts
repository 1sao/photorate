...
kotlin {
  ...
  sourceSets {
    ...
    // iOS + JVM share the kmplitert-backed engine
    // (LiteRTCompiler/TFBuffer).
    // Android keeps the custom CompiledModel code, so
    // kmplitert stays off
    // the android classpath entirely.
    val kmpMain by creating { dependsOn(commonMain.get()) }
    // With only two iOS targets the default hierarchy
    // skips the intermediate
    // iosMain set (it is auto-created when iosX64 is
    // present too), so declare
    // it explicitly to host the shared iOS adapter code
    // in src/iosMain.
    ...
    commonMain.dependencies {
      // The LiteRT provider
      // implements the
      // photosInference contracts
      // (HandLandmarkerFactory +
      // AppClipSearchFactory) so the
      // app can run
      // hand-landmark and MobileCLIP
      // search inference on LiteRT.
      implementation(projects.photosInference)
    }
    kmpMain.dependencies {
      // Cross-platform LiteRT engine
      // (JVM: JNA + bundled dylib;
      // iOS:
      // native C-API). The platform
      // seam below creates
      // LiteRTCompiler
      // instances per target.
      implementation("io.github.leitingzi:kmplitert-core:0.1.4")
      // runBlocking bridges the
      // suspend kmplitert API to the
      // sync pipeline.
    ...
    androidMain.dependencies {
      ...
      // with the OpenCL/GL GPU
      // accelerator. 2.1.6 is the
      // version the
      ...
      // factories mirrors
      // photosOnnx/
      // photosMediaPipe (used by the
      // app's androidTest dataset
      // tests).
