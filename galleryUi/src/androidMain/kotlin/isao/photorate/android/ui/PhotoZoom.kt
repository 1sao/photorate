...
/**
 * Reusable photo-viewer interaction state: a scale + pan pair that powers the
 * [Modifier.panAndZoom] modifier. Gestures write [scale]/[offset] directly
 * (snappy, finger-tracked); programmatic changes (double-tap zoom, reset) go
 * through [animateTo] for a smooth spring.
 *
 * Pan is clamped so a zoomed image never reveals empty space around it: the
 * clamp uses [viewportSize] (the modifier's bounds) and [contentSize] (the
 * drawn image bounds, i.e. the fit rect), both fed by the caller.
 */
@Stable
class PhotoZoomState(val maxScale: Float = MAX_SCALE) {
    ...
    /**
     * Applies a pinch-zoom gesture, keeping the focal [centroid] (in viewport
     * coordinates) pinned under the fingers.
     */
    ...
    /**
     * Smoothly animates to [targetScale] with [targetOffset]. Used by double-tap
     * zoom (keep the tap point pinned) and reset.
     */
...
@Composable
fun rememberPhotoZoomState(maxScale: Float = PhotoZoomState.MAX_SCALE): PhotoZoomState = remember { PhotoZoomState(maxScale) }
 ...
 *  - [onTap] for a single tap (fires only when not zoomed; zoomed taps reset
 *    the zoom instead, like a photo gallery).
 *
 * The gesture handlers never consume drag events beyond the transform, so
 * parent scrollables keep working at 1x zoom.
 *
 * Requires [PhotoZoomState.updateContentSize] to be fed the drawn image size
 * (the fit rect) for correct pan clamping.
 */
@Composable
fun Modifier.panAndZoom(state: PhotoZoomState, onTap: (() -> Unit)? = null): Modifier {
    ...
    return this
        .onSizeChanged { state.updateViewportSize(it) }
        .pointerInput(state) {
