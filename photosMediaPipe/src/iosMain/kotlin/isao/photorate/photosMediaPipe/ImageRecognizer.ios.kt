...
/**
 * iOS [HandLandmarkerFactory] backed by MediaPipe Tasks (SwiftTasksVision).
 * The common contract's [LandmarkCandidate] is a UIImage on iOS; MediaPipe
 * wants an MPPImage, so detect() wraps it here and scales the normalized
 * landmarks back to image points (see the Android counterpart for why).
 */
...
class IosMediaPipeHandLandmarkerFactory : HandLandmarkerFactory {
    override fun createFromOptions(options: HandLandmarkerOptions): HandLandmarker {
        val modelPath = NSBundle.mainBundle.pathForResource("hand_landmarker", ofType = "task")!!
        val mppOptions = MPPHandLandmarkerOptions().apply {
        ...
        measureTime {
            landmarker = MPPHandLandmarker(mppOptions, null)
        }.also {
            NSLog("IosMediaPipeHandLandmarkerFactory initialized in $it ms")
        }
...
@OptIn(ExperimentalForeignApi::class)
class IosMediaPipeHandLandmarker internal constructor(private val landmarker: MPPHandLandmarker) : HandLandmarker {
    override fun detect(candidate: LandmarkCandidate): LandmarkedImage {
        ...
        val detectedIn = measureTime {
            result = landmarker.detectImage(MPPImage(candidate, null), null)!!
        }
    ...
    override fun close() {
        // MPPHandLandmarker has no close() in the SwiftTasksVision interop.
    }
}
