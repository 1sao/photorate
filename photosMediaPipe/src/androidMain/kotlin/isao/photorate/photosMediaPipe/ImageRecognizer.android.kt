...
/**
 * Android [HandLandmarkerFactory] backed by MediaPipe Tasks
 * (HandLandmarker). The common contract's [LandmarkCandidate] is a Bitmap on
 * Android; MediaPipe's native input is an MPImage, so detect() wraps it here.
 *
 * The MediaPipe pipeline historically returned landmarks already normalized to
 * 0..1; the common scan contract expects image-pixel space (the classifier is
 * tuned on isotropic geometry), so detect() scales them back to pixels. The
 * scan then normalizes them again for storage — an exact round trip.
 */
class AndroidMediaPipeHandLandmarkerFactory @Inject constructor(private val context: Context) : HandLandmarkerFactory {
    ...
    private fun createHandLandmarker(options: HandLandmarkerOptions, delegate: Delegate): MpHandLandmarker = try {
        val mediaPipeOptions = MpHandLandmarker.HandLandmarkerOptions.builder()
            .setBaseOptions(
                BaseOptions.builder()
        ...
        measureTimeMillis {
            ...
            Log.d("HandLandmarker", "Initialized with $delegate in $it ms")
        }
        recognizer
    ...
    companion object {
        ...
        const val MODEL_PATH = "composeResources/photorate.shared.generated.resources/files/hand_landmarker.task"
    }
}

class AndroidMediaPipeHandLandmarker internal constructor(private val landmarker: MpHandLandmarker) : HandLandmarker {
