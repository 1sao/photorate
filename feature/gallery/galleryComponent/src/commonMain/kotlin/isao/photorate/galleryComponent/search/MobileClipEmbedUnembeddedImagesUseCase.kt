...
/**
 * [EmbedUnembeddedImagesUseCase] backed by the MobileCLIP vision encoder ([AppClipSearch]).
 *
 ...
 */
@Factory
class MobileClipEmbedUnembeddedImagesUseCase(
      ...
      withContext(Dispatchers.IO) {
        ...
        searchFactory.createFromOptions(AppClipSearchFactory.Options()).use { clip ->
          forEachAccumulating(missing) { image ->
            val candidate = imageLoader.load(image.uri, DECODE_MIN_DIM)
            val embedding = clip.embedImage(candidate)
