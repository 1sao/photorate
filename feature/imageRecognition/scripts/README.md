...
|----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|
| `rtm_pipeline.py`                | Shared RTMDet+RTMPose pipeline with two backends: `onnx` (rtmlib on `ml/original_models/onnx/*.onnx`, ground-truth oracle) and `tflite` (ai-edge-litert on `ml/litert/converted/*_f32.tflite`, device-faithful).        | `LiteRtHandLandmarker.kt`                                                  |
...
| `strip_simcc_postprocess.py`     | Regenerates `ml/original_models/onnx/rtmpose_hand_rawsimcc.onnx` by stripping the PINTO baked SimCC decode (whose `bboxes_width_height` rescale convention differs from rtmlib's). Required by the ONNX oracle backend. | —                                                                          |
