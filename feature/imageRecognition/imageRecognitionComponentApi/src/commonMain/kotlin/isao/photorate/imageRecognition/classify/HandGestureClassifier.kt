...
import isao.photorate.imageRecognition.classify.HandGestureClassifier.tipDirection
...
/**
 * Classifies a detected hand into a rating gesture, or returns null when the hand does not form a
 * recognizable rating gesture (gripping, folded, holding something) — those hands are excluded and
 * receive no score.
 *
 * All measurements are distance ratios or angles relative to the hand's own size, so they are
 * scale- and rotation-invariant. Thresholds were tuned against the sample images in plans/samples
 * (see HandLandmarkDatasetTest).
 */
object HandGestureClassifier {
  ...
  fun classify(hand: Hand): GestureClassification? {
    ...
    // Partial-hand fallback: a hand found only at the
    // image edge by the
    // ONNX pipeline's edge fallback (the thumb is in
    // shot, the rest off-
    // frame — see LandmarkedImage.Hand.edgeDetected).
    // RTMPose hallucinates
    // the off-frame fingers, so the normal all-curled
    // gate below can never
    // fire; the thumb chain is the only trustworthy part.
    // The guess is the
    // THUMBS tier of the tip direction; the landmarker
    // marks such hands
    // uncertain so the UI surfaces them as low-certainty.
    // Gates tuned on
    // plans/samples: confident thumb chain (mean kp1..4
    // >= 0.45), fingers
    // unreliable (mean kp5..20 < 0.40 — holding hands
    // have confident
    // fingers and never qualify), thumb pointing up-ish
    // (angle <= 45).
    if (
      hand.edgeDetected &&
    ...
    val thumbLengthRatio =
      dist(
        p[THUMB_MCP],
        p[THUMB_TIP],
      ) / handSize
    val thumbIndexTipDistance =
      dist(
        p[THUMB_TIP],
        p[INDEX_TIP],
      ) / handSize
    ...
    val thumbFingerAngle =
      angleBetween(
        thumbDirection(p),
        indexDirection(p),
      )

    // OK sign: the thumb and index must form a CLOSED
    // CIRCLE that faces
    // the camera — not merely touching fingertips. Both
    // fingers must be
    // visibly bent into a ring (curl well above 1 =
    // straight), the
    // projected ring must be near-round (circularity),
    // and BOTH ring
    // dimensions must be substantial (a thin sliver is an
    // edge-on/parallel
    // loop, i.e. not facing the camera). Remaining
    // fingers extended.
    // Thresholds tuned in
    // plans/benchmarks/ok_circle_features.py: real OK
    // signs have thumb_curl >= 1.09, index_curl >= 1.17,
    // circularity
    // >= 0.55 and both ring sides >= 0.36; no_score
    // folded/holding hands
    // sit at curl ~1.00-1.12, circ <= 0.24, or a
    // degenerate sliver ring.
    if (
      thumbIndexTipDistance < OK_SIGN_MAX_THUMB_INDEX_DIST &&
        middleExtended &&
        ringExtended &&
        pinkyExtended &&
    ...
    // Thumbs-up/down: fingers curled, thumb extended away
    // from them.
    // The thumb's rotation from image-up communicates the
    // score. The
    // entry thresholds are relaxed (thumb length > 0.4,
    // thumb/finger angle
    // > 85) so rotated/thumb-down gestures (1_coffee,
    // 5_kenya) rate too;
  ...
  private fun thumbCurl(p: List<Point>): Float {
    ...
    return (dist(p[THUMB_MCP], p[THUMB_IP]) +
      dist(
        p[THUMB_IP],
        p[THUMB_TIP],
      )) / straight
  ...
  private fun indexCurl(p: List<Point>): Float {
    ...
    if (straight <= 0f) return 0f
    return (dist(p[INDEX_MCP], p[INDEX_PIP]) +
      dist(
        p[INDEX_PIP],
        p[INDEX_DIP],
      ) +
  ...
  /**
   * Perpendicular distance from the tip contact point (midpoint of thumb tip and index tip) to the
   * ring's base line (thumb MCP -> index MCP), normalized by hand size. A camera-facing circle has
  ...
  private fun isExtended(
    ...
    pipIndex: Int,
    tipIndex: Int,
  ): Boolean = dist(p[tipIndex], p[WRIST]) > dist(p[pipIndex], p[WRIST])

  /**
   * The thumb's away-from-fingers vector (MCP -> TIP), used by the gesture gates. Deliberately NOT
   * the tip segment: the gate measures "thumb away from the curled fingers", which is robust with
   * kp2; switching it to the tip segment changed rotated-read outcomes (1_coffee rated THUMBS-5).
   */
  private fun thumbDirection(p: List<Point>): Point = vector(p[THUMB_MCP], p[THUMB_TIP])

  /**
   * The thumb's POINTING direction — the IP -> TIP segment (kp3->kp4). The CMC/MCP joints sit at
   * the palm and are noisy (low kp confidence on many hands), and the thumb is often curved, so a
   * fit through the whole chain misreads where the tip points (LSQ through kp1..4 flips
   * 5_buco/5_kenya/ 5_vertical to THUMBS-1). The tip segment is the model's most confident part of
   * the thumb and fixes 1_tuna's borderline 147.7/153 deg angle to its label's ONE without changing
   * any other sample.
   */
  ...
  private fun vector(from: Point, to: Point): Point =
    Point(to.x - from.x, to.y - from.y, to.z - from.z)
  ...
  /**
   * The thumb's pointing direction as an angle from image-up (0 = pointing up, 180 = pointing
   * down), measured on the IP -> TIP segment (see [tipDirection]). Public so the dev-mode
   * best-guess rater can reuse the same score tiers for hands that form no recognized gesture.
   */
  ...
  /**
   * The THUMBS score tier for [angle] degrees from image-up. Shared by the classifier and the
   * dev-mode best-guess rater's fallback.
   */
  fun scoreForThumbAngleDegrees(angle: Float): Score =
    when {
