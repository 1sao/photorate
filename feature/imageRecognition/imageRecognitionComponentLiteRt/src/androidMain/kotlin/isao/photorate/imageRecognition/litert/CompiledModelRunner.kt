...
// Vendored from common/kotlin/CompiledModelRunner.kt — edit the canonical and run
// tools/sync_common.py --apply.
 ...
 * native memory even when the model itself is closed.
 *
 ...
 * - [run] enqueues work on the GPU and may return before the computation has finished; the readback
 *   ([readOutput]) is the call that waits. Always benchmark run + readback together, never run()
 *   alone.
...
class CompiledModelRunner private constructor(private val model: CompiledModel) : AutoCloseable {
  ...
  companion object {
    /**
     * Compiles a model bundled in `assets/`. The module's Gradle config must set `aaptOptions {
     * noCompress += "tflite" }` so the asset stays mmappable.
     */
    fun fromAssets(
      context: Context,
      fileName: String,
      ...
      CompiledModelRunner(
        CompiledModel.create(
          context.assets,
          fileName,
          CompiledModel.Options(*accelerators),
          null,
        )
      )

    /**
     * Compiles a bundled asset with caller-provided options (e.g. GpuOptions backend selection for
    ...
    fun fromAssets(
      ...
      CompiledModelRunner(CompiledModel.create(context.assets, fileName, options, null))

    /**
     * Compiles a model from an absolute file path — the pattern for large models staged into the
     * app's `filesDir` by an install script.
     */
    fun fromFile(
      path: String,
      vararg accelerators: Accelerator = arrayOf(Accelerator.GPU),
    ): CompiledModelRunner =
      CompiledModelRunner(CompiledModel.create(path, CompiledModel.Options(*accelerators), null))
  }
