...
import isao.photorate.imageRecognition.litert.LiteRtRtmModels.NUM_LANDMARKS
import isao.photorate.imageRecognition.litert.LiteRtRtmModels.SIMCC_SPLIT_RATIO
import kotlin.math.min

/**
 * Model contract for the LiteRT-converted RTM hand models (see `ml/litert/` recipe):
 *
 * - Detector: RTMDet-nano hand, `end2end_f32clean.onnx` -> `rtmdet_hand_320_f32.tflite`. float32
 *   NCHW input `input` [1,3,320,320]; raw anchor outputs boxes [1,2100,4] + scores [1,2100,1] in
 *   320-space (the baked NMS + sort tail were cut caller-side — the app's
 *   `AndroidOnnxHandLandmarker.detectBoxes()` NMS params are the original export's: iou=0.6,
 *   score_thr=0.05, max_out=200).
 * - RTMPose: RTMPose-m hand SimCC, `end2end_f32clean.onnx` -> `rtmpose_hand_256_f32.tflite`.
 *   float32 NCHW input `input` [1,3,256,256]; raw SimCC outputs simcc_x/simcc_y [1,21,512] (split
 *   ratio 2.0, decoded caller-side).
 *
 * Verification of the conversions lives in `LiteRtOnDeviceVerificationTest` (app androidTest)
 * against reference dumps in `ml/litert/refs` (host-generated, see
...
object LiteRtRtmModels {
   ...
   * `AndroidOnnxHandLandmarker.rtmposeLandmarks()` decode: per-axis argmax over 512 bins divided by
   * [SIMCC_SPLIT_RATIO], confidence = min of the two peaks. Returns [NUM_LANDMARKS] * 3 floats: x,
   * y, conf.
   */
  ...
  /**
   * Caller-side NMS over the raw anchors (iou=0.6 / thr=0.05 / max 200, exactly the baked export's
   * params). Returns kept indices in descending-score order.
   */
