 ...
 *
 * Embeddings are precomputed ahead of time by [PopulateImageEmbeddingsUseCase] and stored in the
 * `ImageEmbedding` table for hand images only, so a search only embeds the query text and ranks the
 * stored vectors by cosine similarity — it never re-embeds gallery images.
 *
 * The CLIP session (LiteRT or ONNX, whichever AppClipSearchFactory the root PlatformModule
 * provides) is created lazily and reused across searches: model load + session creation is
 * expensive, and the `@Factory` instance is ViewModel-scoped so the cache lives exactly as long as
 * the screen.
 */
@Factory
  ...
  private fun session(): AppClipSearch {
    ...
    synchronized(sessionMutex) {
      return cachedSession
        ?: searchFactory.createFromOptions(AppClipSearchFactory.Options()).also {
          cachedSession = it
        }
    }
  }
