...
/**
 * Platform image a hand-landmark model consumes: a Bitmap on Android, a UIImage on iOS. Each
 * provider adapts it to its native input internally (MediaPipe wraps it in an MPImage; ONNX
...
/**
 * Model-agnostic options for a hand-landmark run. Fields are per-model: each provider reads the
 * ones it knows and ignores the rest (MediaPipe uses the presence confidence, ONNX the keypoint
 * floors).
 */
 ...
 * aspect. The scan un-rotates (via [LandmarkedImage.Hand.rotationDegrees], 0 for MediaPipe) and
 * normalizes to 0..1 for storage.
 */
...
/**
 * The on-device hand-landmark models the app ships. Each provider module implements
 * [HandLandmarkerFactory] for its model; [LandmarkerFactoryProvider] resolves the active one.
 */
...
/**
 * The selection seam for switching between local models: resolves a model to its factory.
 * Implemented per platform in `shared`'s PlatformModule — swap [defaultModel] there to change what
 * a scan runs (Android defaults to ONNX, iOS to MediaPipe).
 */
 ...
 * The platform module builds it with its own factory implementations (ONNX on Android, MediaPipe on
 * iOS).
 */
