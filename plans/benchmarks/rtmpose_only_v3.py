#!/usr/bin/env python3
"""RTMPose-only v3 — no sparse model, no palm rotation model.

Architecture: RTMDet -> {0,90,180,270} rotations -> RTMPose -> 
  strict gesture classifier (extended fingers must also be straight)

The key insight: the sparse model's presence gate is replaced by a stricter
gesture classifier. For a finger to count as "extended" it must be BOTH:
  1. Tip farther from wrist than PIP (the current is_extended check)
  2. PIP-to-tip distance < 0.25 * hand_size (finger-straightness check)

This filters out hands holding objects (curled fingers around bag) and
folded hands, while keeping real rating gestures.

Requires: pip install onnxruntime opencv-python-headless numpy
Usage: python3 rtmpose_only_v3.py
"""
import cv2
import math
import numpy as np
...
def decode_640(path):
    ...
    return cv2.resize(img, (iw // s, ih // s)) if s > 1 else img
...
def rotate_and_crop_square(img, cx, cy, side, degree):
    ...
    if x0 < 0 or x0 + bw > padded.shape[1] or y0 < 0 or y0 + bh > padded.shape[0]:
        return None
    ...
    if x0f < 0 or x0f + rw > rotated.shape[1] or y0f < 0 or y0f + rh > rotated.shape[0]:
        return None
...
def finger_straight(p, pip_i, tip_i, hand_size):
    """PIP-to-tip distance / hand_size — smaller = straighter."""
    return dist(p[tip_i], p[pip_i]) / hand_size


def is_extended_and_straight(p, pip_i, tip_i, hand_size, max_curl=0.25):
    """A finger is 'truly extended' = extended + straight (not curled)."""
    return (is_extended(p, pip_i, tip_i) and
            finger_straight(p, pip_i, tip_i, hand_size) < max_curl)


def classify(p, max_curl=0.25):
    """Classifier with finger-straightness check. For a finger to count as
    extended it must be both extended (tip > PIP from wrist) AND straight
    (PIP-to-tip < max_curl * hand_size)."""
    hand_size = dist(p[0], p[9])
    if hand_size <= 0: return None
    thumb_len = dist(p[2], p[4]) / hand_size
    thumb_index = dist(p[4], p[8]) / hand_size
    idx_ext = is_extended_and_straight(p, 6, 8, hand_size, max_curl)
    mid_ext = is_extended_and_straight(p, 10, 12, hand_size, max_curl)
    ring_ext = is_extended_and_straight(p, 14, 16, hand_size, max_curl)
    pink_ext = is_extended_and_straight(p, 18, 20, hand_size, max_curl)
    tips = [p[8], p[12], p[16], p[20]]
    spread = sum(dist(tips[i], tips[i + 1]) for i in range(3)) / 3 / hand_size
    tdir = vec(p[2], p[4])
    idir = vec(p[5], p[8])
    tf_angle = angle_between(tdir, idir)
    # OK sign: thumb tip touches index tip, remaining fingers extended & straight.
    if thumb_index < 0.2 and mid_ext and ring_ext and pink_ext:
        return ("OK_SIGN", 3)
    # Peace/V sign: index and middle extended/straight, ring and pinky curled.
    if idx_ext and mid_ext and not ring_ext and not pink_ext:
        return ("PEACE", 2)
    # Rock sign: index and pinky extended/straight, middle curled.
    if idx_ext and pink_ext and not mid_ext:
        return ("ROCK", 5)
    # Open palm: all fingers extended & straight and splayed.
    if idx_ext and mid_ext and ring_ext and pink_ext and spread > 0.2:
        return ("OPEN_PALM", 3)
    # Thumbs up/down: fingers curled, thumb extended away.
    # Use the OLD is_extended for thumbs (since fingers ARE curled, we just
    # need the basic PIP check — don't require straightness).
    # Actually, for thumbs, all fingers must be curled (not extended), so
    # the straightness check is irrelevant.
    idx_ext_old = is_extended(p, 6, 8)
    mid_ext_old = is_extended(p, 10, 12)
    ring_ext_old = is_extended(p, 14, 16)
    pink_ext_old = is_extended(p, 18, 20)
    all_curled = not (idx_ext_old or mid_ext_old or ring_ext_old or pink_ext_old)
...
def main():
    # Test with different max_curl thresholds
    for max_curl in [0.25, 0.20]:
        print(f"\n{'=' * 80}")
        print(f"  max_curl = {max_curl}")
        print(f"{'=' * 80}")
        print(f"{'sample':<38} {'RTMPose-only v3':<44}")
        correct = 0
        total_scored = 0
        false_positives = []
        ...
        print(f"\naccuracy {correct}/{total_scored}, false positives: {false_positives}")
