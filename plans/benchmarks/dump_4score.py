#!/usr/bin/env python3
"""Dump per-rotation RTMPose classifications for the 4-score samples.

The hybrid got 4/4.jpg -> THUMBS-4 via the sparse-best rotation; RTMPose-only
reads THUMBS-5 at deg 0 (image space). This dumps every rotation x factor so
we can see whether some rotation reads the correct THUMBS-4 without the sparse
model steering it.
"""
import cv2
import math
...
def classify(p):
    ...
    if thumb_index < 0.2 and mid_ext and ring_ext and pink_ext:
        ...
        thumb_curl = (dist(p[2], p[3]) + dist(p[3], p[4])) / dist(p[2], p[4]) if dist(p[2], p[
            4]) > 0 else 0.0
        index_curl = (dist(p[5], p[6]) + dist(p[6], p[7]) + dist(p[7], p[8])) / dist(p[5],
                                                                                     p[8]) if dist(
            p[5], p[8]) > 0 else 0.0
        if thumb_curl >= 1.08 and index_curl >= 1.15 and circularity >= 0.40:
            return ("OK_SIGN", 3)
    ...
    if all_curled and thumb_len > 0.25 and tf_angle > 85:
        ...
        return ("THUMBS", score, angle)
    return None


def main():
    for fpath in ["4/4.jpg", "4/4_also.jpg", "5/5_kenya.jpg", "5/5_trope.jpg", "5/5_also.jpg",
                  "1/1_coffee.jpg"]:
        f = SAMPLES / fpath
        img = decode_640(f)
        iw, ih = img.shape[1], img.shape[0]
        ...
        for bi, box in enumerate(boxes[:MAX_HANDS]):
            print(f"  box{bi} det={box[4]:.3f}")
            if box[4] < MIN_DET:
                continue
            palm_deg = palm_rotation(img, box[:4])
            print(f"  palm_deg={palm_deg:.1f}")
            for factor in (1.2, 0.85):
                ...
                for deg in sorted(set([0, 90, 180, 270, palm_deg])):
                    ...
                    cl = classify(p)
                    hs = dist(p[0], p[9])
                    label = f"{cl[0]}-{cl[1]}(a={cl[2]:.0f})" if cl and len(cl) == 3 else (
                        f"{cl[0]}-{cl[1]}" if cl else "unrated")
                    print(f"    factor={factor} deg={deg:>6} kp={kp_mean:.2f} {label}")
