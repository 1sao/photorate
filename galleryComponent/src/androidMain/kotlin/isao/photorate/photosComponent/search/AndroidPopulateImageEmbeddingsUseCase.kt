...
/**
 * Android [PopulateImageEmbeddingsUseCase] backed by the MobileCLIP vision encoder
 * ([AppClipSearch]).
 *
 * For every hand image missing an embedding it decodes a scaled-down version of the photo (same
 * sample-size strategy as the ONNX image loader) and embeds it. The vision model only ever sees a
 * 224x224 center crop, so a full-resolution decode is unnecessary: a scaled-down decode embeds
 * almost identically (verified on-device by MobileClipSearchDatasetTest and in Python on the same
        ...
        measureTime {
          ...
          missing.forEach { image ->
            ...
            val embedding = runCatching {
              clip.embedImage(bitmap.toJpegBytes())
            }
              .getOrElse { error ->
          ...
          .also { log.i { "Image embeddings computed in ${it.inWholeMilliseconds} ms" } }
      }
      Unit
    }
  ...
  private fun decodeScaled(uri: String): Bitmap? =
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        ...
        val inputStream = contentResolver.openInputStream(uri.toUri()) ?: return null
