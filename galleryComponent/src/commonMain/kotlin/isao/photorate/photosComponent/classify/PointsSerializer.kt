 ...
 * Format: [count: Int (4 bytes LE)] [x: Float, y: Float, z: Float] * count (each 4 bytes LE)
 * Total size: 4 + 12 * count bytes.
 */
object PointsSerializer {
    ...
    fun serialize(points: List<Point>): ByteArray {
        ...
        var offset = 4
        for (point in points) {
            writeFloat(bytes, offset, point.x)
            offset += 4
            writeFloat(bytes, offset, point.y)
            offset += 4
            writeFloat(bytes, offset, point.z)
            offset += 4
        }
        return bytes
    }
    ...
    fun deserialize(bytes: ByteArray): List<Point> {
        ...
        repeat(count) {
            ...
            offset += 4
            val y = readFloat(bytes, offset)
            offset += 4
            val z = readFloat(bytes, offset)
            offset += 4
    ...
    private fun readInt(bytes: ByteArray, offset: Int): Int = (bytes[offset].toInt() and 0xFF) or
        ((bytes[offset + 1].toInt() and 0xFF) shl 8) or
        ((bytes[offset + 2].toInt() and 0xFF) shl 16) or
        ((bytes[offset + 3].toInt() and 0xFF) shl 24)
}
