...
kotlin {
  ...
  sourceSets {
    ...
    getByName("androidHostTest").dependencies {
      ...
      // JDBC SQLite driver so the
      // host tests can exercise the
      // real schema.
...
sqldelight {
  databases.create("PhotoRateDb") {
    ...
    // Merges the config schema into this database (the
    // gallery queries JOIN
    // the config table). The generated gallery
    // PhotoRateDb therefore also
    // carries the config tables and requires
    // configComponent's adapters.
    dependency(project(":configComponent"))
    // The schema uses UPSERT (INSERT ... ON CONFLICT ...
    // DO UPDATE, SQLite
    // >= 3.24), which the default 3.18 dialect cannot
    // parse. 3.30 is the
    // first published dialect >= 3.24 and matches the
    // bundled SQLite of our
