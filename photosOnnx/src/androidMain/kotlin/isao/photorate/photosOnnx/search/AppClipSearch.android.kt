 ...
 * ([isao.photorate.inference.classify.HandLandmarkerFactory]): the factory is Android-specific,
 * injected with a [Context], and registered in Koin from the shared module's
...
class AndroidAppClipSearchFactory @Inject constructor(private val context: Context) :
  ...
  override fun createFromOptions(options: AppClipSearchFactory.Options): AppClipSearch {
    ...
    measureTimeMillis {
      ...
      .also { Log.d(TAG, "Initialized MobileCLIP search in $it ms") }
    return session
  }
...
/**
 * Android MobileCLIP session: two ONNX Runtime sessions (text + vision encoders) plus the CLIP
 * tokenizer, all loading their assets from the app. Models are copied from assets to filesDir on
 * first use so ORT can mmap them.
 */
class AndroidAppClipSearch
internal constructor(
  ...
  init {
    ...
    val tokenizerJson =
      context.assets.open(AndroidAppClipSearchFactory.TOKENIZER_ASSET).bufferedReader().use {
        it.readText()
      }
