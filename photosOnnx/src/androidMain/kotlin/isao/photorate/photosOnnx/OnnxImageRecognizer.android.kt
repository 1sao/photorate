package isao.photorate.photosOnnx
...
import isao.photorate.photosOnnx.AndroidOnnxHandLandmarker.Companion.NMS_MAX_OUT
import java.io.File
...
/**
 * Android factory for the ONNX hand pipeline:
 * RTMDet hand detection -> multi-rotation RTMPose search -> gesture rating.
 *
 * The sparse landmark model (presence gate) and the palm rotation model were
 * removed: the rater is the gate now (the rotation search passes through
 * gesture-less hands so the dev-mode best-guess rater can still store them
 * for inspection). Verified against plans/samples in
 * plans/benchmarks/rtmpose_only_v5.py — the tightened gesture set (THUMBS /
 * ROCK / OK-as-a-camera-facing-circle) filters every no_score sample on its
 * own, and RTMPose rates all of the score samples (2/ and 3_open_hand_palm_down
 * intentionally no longer rate — PEACE/OPEN_PALM were dropped as false
 * positives).
 */
class AndroidOnnxHandLandmarkerFactory @Inject constructor(
    ...
    override fun createFromOptions(options: HandLandmarkerOptions): HandLandmarker {
        ...
        measureTimeMillis {
            ...
            Log.d(TAG, "Initialized ONNX hand pipeline in $it ms (provider=${if (nnapiFlags != null) "NNAPI" else "CPU"})")
        }
        return landmarker
    }
    ...
    companion object {
        ...
        /**
         * Parses a comma-separated [NNAPIFlags] string (e.g. "USE_FP16,CPU_DISABLED")
         * for the instrumented benchmark: blank or "default" = plain
         * addNnapi() defaults, null input = plain CPU. Lets the same
         * pipeline run under any provider without the app depending on ORT
         * types (used by OnnxHandLandmarkDatasetTest via the `nnapiFlags`
        ...
        const val RTMPOSE_ASSET = "rtmpose-m_simcc-hand5_pt-aic-coco_210e-256x256-74fb594_20230320/end2end_f32clean.onnx"
    }
}

/**
 * Android ONNX pipeline: RTMDet letterbox -> multi-rotation RTMPose search ->
 * gesture classifier (the gate). Verified on plans/samples with onnxruntime
 * 1.28.0 — the same version the Android AAR ships.
 */
class AndroidOnnxHandLandmarker internal constructor(
    context: Context,
    ...
    private fun detectBoxes(bitmap: Bitmap): List<Pair<IntArray, Float>> {
        ...
        val (boxesRaw, scoresRaw) = OnnxTensor.createTensor(ortEnv, input, shape).use { tensor ->
    ...
    /**
     * Port of the baked NonMaxSuppression (params read from the original
     * export's initializers): filter by score, then suppress overlapping
     * boxes in descending-score order, keep at most [NMS_MAX_OUT]. Returns
     * indices into the raw anchor arrays. The early `break` on a sub-
     * threshold score is valid only because [order] is sorted descending —
     ...
     * undistorted regardless of image aspect ratio — image-space
     * normalization (x/iw, y/ih) would anisotropically squash the geometry on
     * non-square images and reject valid hands (verified: 3_ok_sign_peanuts
     * fails normalized but rates OK in pixels). Storage code un-rotates via
     * [LandmarkedImage.Hand.rotationDegrees] and normalizes to 0..1.
     * Confidence tiers are keypoint-based: below
     * [HandLandmarkerOptions.minHandKpConfidence] a box is not a hand at all;
     * ROCK/OK below [HandLandmarkerOptions.minHandConfidentKpConfidence] are
     * still rated but flagged uncertain (best guess) — tuned so
     * low-confidence ROCK/OK reads (holding-in-two-hands at kp 0.28) filter
     * while real ones (kp >= 0.58) stay confident.
     */
     ...
     * Python). The original end2end.onnx has NO baked SimCC decode (the old
     * modified export had one): it outputs raw heatmaps simcc_x/simcc_y
     * [1, 21, 512], so decode = argmax / simcc_split_ratio (2.0) mapped back
     * through the inverse affine to crop pixels, with min(peak_x, peak_y) as
     * confidence. Verified bit-exact against the old decode-baked export on
     * plans/samples (keypoint diff 0.0000, score diff 0.00000).
     */
    private fun rtmposeLandmarks(crop: Bitmap): FloatArray? {
        ...
        val (simccX, simccY) = OnnxTensor.createTensor(ortEnv, input, shape).use { tensor ->
    ...
    /**
     * Mean/std CHW for the RTMDet detector (RGB order) and RTMPose (BGR order).
     * Mean/std are the mmdet RGB values (123.675, 116.28, 103.53); RTMPose
     * receives BGR-order pixels normalized with those same stats — exactly the
     * verified Python preprocessing.
     */
