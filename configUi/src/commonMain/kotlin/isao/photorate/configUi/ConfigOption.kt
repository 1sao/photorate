...
/**
 * One group of related settings on the Config screen. The setup is static and
 * shared between the Android Compose UI and the (future) iOS UI; only the
 * option values derive from the current [GalleryConfig].
 */
data class ConfigSection(val title: String, val options: List<ConfigOption>)

/**
 * A single user-adjustable option. Every option knows its own [name] and can
 * render its current value from a [GalleryConfig].
 */
...
/**
 * A mutually-exclusive choice, rendered as radio buttons that wrap into a row
 * when they fit on screen.
 */
data class RadioOption(override val name: String, val choices: List<RadioChoice>) : ConfigOption {
    override fun currentValue(config: GalleryConfig): String = choices.firstOrNull { it.isSelected(config) }?.label.orEmpty()

    data class RadioChoice(val label: String, val isSelected: (GalleryConfig) -> Boolean, val intent: () -> ConfigIntent)
}

/**
 * A single-thumb slider, edited in a dialog. [description], when set, is shown
 * in the dialog.
 */
...
/**
 * A two-thumb range slider, edited in a dialog. [description], when set, is
 * shown in the dialog.
 */
...
val defaultConfigSections: List<ConfigSection> = listOf(
    ConfigSection(
        ...
        options = listOf(
            ...
            SliderOption(
                ...
                steps = 98,
                description = "Only show photos where the hand covers at least this percentage of the image.",
                valueRange = 1f..100f,
