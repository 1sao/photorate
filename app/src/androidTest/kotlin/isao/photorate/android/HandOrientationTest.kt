...
/**
 * Decisive experiment: does the palm detector see these hands in ANY orientation? Crops the known
 * hand regions (identified from renders), then tries the crop as-is plus mirrored and
 * 90/180/270-degree rotations, each upscaled to 512px. If any orientation is detectable, a
 * multi-orientation pipeline can recover the hands; if none are, the model itself cannot see them
 * (dorsal view) and a different detector model is required.
 */
...
class HandOrientationTest {
  ...
  fun orientations() {
    ...
    try {
      for ((file, region) in REGIONS) {
        ...
        // Pad
        // generously so rotation keeps the hand in frame.
        ...
        for ((name, v) in variants) {
          ...
          if (hands.isNotEmpty()) {
            ...
            hands.forEachIndexed { i, hand ->
              ...
              log(
                "$file|$name hand=$i|gesture=$gesture|score=$score|points=${
                                    dumpPoints(
                                        hand
                                    )
                                }"
  ...
  companion object {
    ...
    private val REGIONS =
      listOf(
        // 4.jpg
        // (3000x4000): hand in right-middle.
        "4/4.jpg" to
          Region(
            0.5f,
            0.28f,
