...
/**
 * Runs the LiteRT hand pipeline (RTMDet + RTMPose on CompiledModel, GPU first with CPU fallback —
 * see [AndroidLiteRtHandLandmarkerFactory]) over the sample dataset in plans/samples, mirroring
 * [HandLandmarkDatasetTest] (MediaPipe) and the ONNX dataset test.
 *
 * The pipeline logic is a port of the verified ONNX pipeline on the same two models (converted to
 * tflite in ml/litert/), so the expectations start from the ONNX on-device results and are
 * re-verified here from real runs — any deviation (e.g. a marginal NMS box or SimCC bin flipping
 * between runtimes) shows up as a failing assertion that must be explained, not silenced.
 */
...
class LiteRtHandLandmarkDatasetTest {
  ...
  fun allSamplesScoreAsExpected() {
    ...
    try {
      for (scoreDir in
        ...
        for (file in files) {
          ...
          val scores =
            detected.hands.mapNotNull { hand ->
          ...
          if (expected == null) {
            ...
            val allUncertain = detected.hands.isNotEmpty() && detected.hands.all { it.uncertain }
            if (allUncertain) {
              uncertainDetections += "$scoreDir/$file"
            } else if (confidentScores.isEmpty() || expected !in confidentScores) {
              scoreMismatches +=
                "$scoreDir/$file: confident=$confidentScores all=$scores (expected $expected)"
            }
          }
        }
      }
    ...
    assertEquals(
      ...
      scoreMismatches.map { it.substringBefore(":") },
    )
  }

  companion object {
    // Calibrated from real on-device runs of the LiteRT
    // pipeline (GPU,
    // OpenCL + FP32 — the factory's default) on the Redmi
    // (Adreno 618)
    // device. Mostly identical to the ONNX baseline; the
    // one runtime
    // delta is 2/2_coffee.jpg (see
    // KNOWN_SCORE_MISMATCHES).
    private val KNOWN_DETECTION_GAPS = listOf<String>()

    // Hands the pipeline DOES detect but the real rater
    // (LandmarkRaterByThumb)
    // rejects. 3_open_hand_palm_down (open palm,
    // PEACE/OPEN_PALM dropped)
    // and 2_peanuts (holding — no gesture forms)
    // intentionally no longer
    // rate. 2_coffee is NOT here (unlike ONNX): on the
    // LiteRT GPU one
    // holding hand reads a confident THUMBS-4 (see
    // KNOWN_SCORE_MISMATCHES).
    private val KNOWN_UNRATED_DETECTIONS =
      listOf(
        "3/3_open_hand_palm_down.jpg",
        "2/2_peanuts.jpg",
      )

    // Samples whose hands are all uncertain-tier
    // (best-guess scores):
    //  5/5_kimbo.jpg — edge thumb-only fallback (thumb in
    // shot, left edge).
    //  4/4_also.jpg — dorsal score-4 hand; ROCK-5 at kp
    // 0.36 < 0.45 gate.
    //  2/2_peanuts.jpg — the detected hands are below the
    // confident gate.
    private val KNOWN_UNCERTAIN_DETECTIONS =
      listOf(
        "5/5_kimbo.jpg",
        "4/4_also.jpg",
        "2/2_peanuts.jpg",
    ...
    // Samples with a CONFIDENT hand that was scored
    // wrong:
    //  2/2_coffee.jpg — NEW for LiteRT GPU: a rotated
    // (180°) read of a
    //    holding hand crosses the THUMBS gates and rates
    // a confident
    //    FOUR (ONNX CPU rejected the same hand). GPU FP32
    // keypoint drift
    //    flips this marginal classification — the same
    // class of
    //    borderline flip the verification README warns
    // about. Documented
    //    as a finding, not a pass.
    //  4/4.jpg — the thumb reads 36° from image-up
    // (borderline FIVE/4
    //    boundary), same as the ONNX pipeline.
    //  3/3_open_hand_palm_down.jpg — the detected hand
    // forms no gesture
    //    (unrated, listed here because the sample isn't
    // fully uncertain).
    private val KNOWN_SCORE_MISMATCHES =
      listOf(
        "4/4.jpg",
        "3/3_open_hand_palm_down.jpg",
        "2/2_coffee.jpg",
  ...
  private fun expectedScore(scoreDir: String): Score? =
    when (scoreDir) {
      "5" -> Score.FIVE
      "4" -> Score.FOUR
      "3" -> Score.THREE
