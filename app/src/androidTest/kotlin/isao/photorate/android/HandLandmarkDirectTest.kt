...
/**
 * Runs the EXTRACTED hand landmark model (hand_landmarks_detector.tflite,
 * the same model MediaPipe runs AFTER its palm detector) DIRECTLY on hand
 * crops, bypassing the palm-detector stage entirely.
 *
 * The landmark model outputs [63 landmarks, presence score, handedness,
 * world landmarks]. If its presence score fires on the exact hand regions
 * that the palm detector misses (4/4.jpg, 4/4_also.jpg, 5/5_horizontal.jpg),
 * then the landmark model CAN see these hands and the fix is a better
 * hand-finder (e.g. a different detector), not a different landmark model.
 *
 * Control: the app pipeline detects 5_vertical's hand, so we crop that
 * detected hand's tight bbox (from its 21 landmarks) and expect HIGH
 * presence through this direct path. Problem regions are swept over several
 * padding factors so a loose region can't cause a false miss.
 */
...
class HandLandmarkDirectTest {
    ...
    fun landmarkModelDirectlyOnCrops() {
        ...
        // TFLite requires a DIRECT byte buffer for the model (heap buffers throw).
        ...
        try {
            ...
            for (region in PROBLEM_REGIONS) {
                ...
                for (pad in PADS) {
                    runDirect(region.file, "problem-${region.label}", cx0, cy0, cx1, cy1, interpreter, pad)
                }
            }
        } finally {
            interpreter.close()
    ...
    /**
     * Control: run the FULL app pipeline on 5_vertical (palm detector works
     * here), take the detected hand's tight landmark bbox, and feed that
     * exact region to the landmark model directly. High presence proves the
     * direct-crop preprocessing is faithful; low presence would mean our
    ...
    private fun runControl(interpreter: Interpreter) {
        ...
        try {
            ...
            // Normalized landmark coords -> pixel bbox.
            ...
            for (pad in PADS) {
                runDirect("5/5_vertical.jpg", "control-5_vertical", x0, y0, x1, y1, interpreter, pad)
            }
        } finally {
            landmarker.close()
        }
    ...
    private fun runDirect(
        ...
        for (normalization in listOf("neg1to1", "0to1")) {
            val input = toInputBuffer(pixels, normalization)
            ...
            interpreter.runForMultipleInputsOutputs(
                arrayOf(input),
                mapOf(0 to landmarks, 1 to presence, 2 to handedness, 3 to world),
            )
            ...
            log(
                "$label|$assetPath|pad=$padFraction|norm=$normalization|presence=%.3f|bbox=%.2fx%.2f|aspect=%.2f|handed=%.2f"
                    .format(score, bw, bh, if (bh > 0f) bw / bh else 0f, handedness[0][0]),
            )
        }
    }

    private fun toInputBuffer(pixels: IntArray, normalization: String): ByteBuffer {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ...
            ImageDecoder.createSource(buffer).let { source ->
                ImageDecoder.decodeBitmap(source) { decoder, info, _ ->
                    val s = min(info.size.width, info.size.height) / DECODE_MAX_DIM
    ...
    private data class Region(val file: String, val label: String, val x0: Float, val y0: Float, val x1: Float, val y1: Float)

    companion object {
        private const val DECODE_MAX_DIM = 1024
        private val PADS = listOf(0.2f, 0.5f)

        // The three hands the palm detector misses (regions from renders).
        private val PROBLEM_REGIONS = listOf(
            Region("4/4.jpg", "4jpg", 0.5f, 0.28f, 0.72f, 0.55f),
            Region("4/4_also.jpg", "4also", 0.22f, 0.15f, 0.62f, 0.55f),
            Region("5/5_horizontal.jpg", "5horiz", 0.0f, 0.12f, 0.25f, 0.6f),
        )
    }
}
