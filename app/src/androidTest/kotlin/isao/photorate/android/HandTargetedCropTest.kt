...
/**
 * Targeted hand-region crops for the 3 undetected samples. The exact hand bounding boxes were
 * identified from ASCII renders of the images. Each region is cropped from a ~1500px decode, padded
 * generously, upscaled to a fixed target size, and run through the real MediaPipe landmarker. This
 * determines whether the fix is a crop+upscale pipeline (landmarker works on a proper crop) or
 * whether the model itself cannot see these hands at all.
 */
...
class HandTargetedCropTest {
          ...
          for (target in
            ...
            if (hands.isNotEmpty()) {
              ...
              hands.forEachIndexed { i, hand ->
                ...
                log(
                  "$file|hand=$i pad=$pad|gesture=$gesture|score=$score|points=${
                                        dumpPoints(
                                            hand
                                        )
                                    }"
  ...
  companion object {
    ...
    private val REGIONS =
      listOf(
        // 4.jpg
        // (3000x4000): hand in right-middle, from render of (1500,800)-(3000,2000).
        "4/4.jpg" to
          Region(
            3000,
            4000,
