...
/**
 * Runs the real MediaPipe hand landmarker (same model, factory and image pipeline as the app — the
 * ORIGINAL pipeline, kept alongside the ONNX one) over the sample dataset in plans/samples and
 * documents — from real on-device runs — which samples it detects AND rates.
 *
 * The ONNX hybrid pipeline ([OnnxHandLandmarkDatasetTest]) rates every sample except 4/4_also and
 * 2/2_peanuts; the old MediaPipe pipeline has no RTMPose fallback, so it additionally cannot rate
 * the newly added rotated/thumb-down gestures (5_trope, 2_coffee's secondary hand, ...). Those
 * honest gaps are asserted below so the test fails loudly if a pipeline change ever shifts them.
 */
...
class HandLandmarkDatasetTest {
        ...
        for (file in files) {
          ...
          val scores =
            detected.hands.mapNotNull { hand ->
              ...
              log(
                "$scoreDir/$file|hand=${detected.hands.indexOf(hand)}|gesture=$gesture|score=$score"
              )
              score
            }
  ...
  companion object {
    // Real on-device gaps for the ORIGINAL MediaPipe
    // pipeline on the full
    // dataset. The palm detector misses these (4/4 and
    // 5_horizontal were
    // already gaps; the ONNX RTMDet replaced the palm
    // detector), and
    // 2_coffee's two hands stay below the presence gate.
    private val KNOWN_DETECTION_GAPS =
      listOf(
        "5/5_horizontal.jpg",
        "4/4.jpg",
        "4/4_also.jpg",
    ...
    // Hands the palm detector DOES find but the rater
    // rejects: 5_trope's
    // MediaPipe sparse geometry never satisfies the
    // gesture classifier
    // (this old pipeline has no RTMPose fallback).
    private val KNOWN_UNRATED_DETECTIONS = listOf("5/5_trope.jpg")
