...
/**
 * Multi-scale crop scan for the 3 undetected samples. The palm detector inside HandLandmarker
 * resizes the whole input to ~192x192, so small hands are lost. Here we slide crops of several
 * sizes across the image and upscale each crop to a fixed TARGET px so the hand fills a large
 * fraction of the detector input. Logs every crop that produces a hand (with landmarks), nothing
 * else.
 */
