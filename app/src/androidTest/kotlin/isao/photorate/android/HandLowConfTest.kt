...
/**
 * Tests whether very low detection confidences (0.2 / 0.15 / 0.1) recover the
 * 3 hands the palm detector misses at conf 0.35, on the known hand-region
 * crops (identified from renders). If the palm detector fires at all, the
 * confidence threshold is the blocker; if not, the model itself cannot see them.
 */
...
class HandLowConfTest {
    ...
    fun lowConfOnCrops() {
        for (conf in listOf(0.2f, 0.15f, 0.1f)) {
            ...
            val landmarker = factory.createFromOptions(
                HandLandmarkerOptions(maxNumHands = 2, minHandDetectionConfidence = conf, minHandPresenceConfidence = conf),
            )
            try {
    ...
    private data class Region(val file: String, val x0: Float, val y0: Float, val x1: Float, val y1: Float)

    companion object {
        private const val DECODE_MAX_DIM = 1024

        private val REGIONS = listOf(
            Region("4/4.jpg", 0.5f, 0.28f, 0.72f, 0.55f),
            Region("4/4_also.jpg", 0.22f, 0.15f, 0.62f, 0.55f),
            Region("5/5_horizontal.jpg", 0.0f, 0.12f, 0.25f, 0.6f),
        )
    }
}
