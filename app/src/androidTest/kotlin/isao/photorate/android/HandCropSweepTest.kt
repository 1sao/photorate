...
/**
 * Two-stage validation for the 3 samples the palm detector misses on the full
 * image (4/4.jpg, 4/4_also.jpg, 5/5_horizontal.jpg):
 *
 *  Stage 1 — localize the hand by tiling the image (overlapping grid). The palm
 *  detector inside HandLandmarker internally resizes the whole input to ~192x192,
 *  so a small hand gets compressed away; a tile makes the hand a large fraction.
 *
 *  Stage 2 — for every tile that produced a hand, crop the region from the
 *  full-res bitmap, upscale it to ~512px, and re-landmark the crop. Small tiles
 *  yield poor landmarks (classifier returns null), so we re-run on a proper crop.
 *
...
class HandCropSweepTest {
    ...
    fun sweepTilesAndCrops() {
        ...
        try {
            for (file in listOf("4/4.jpg", "4/4_also.jpg", "5/5_horizontal.jpg")) {
                ...
                for (tileDiv in listOf(3, 4, 5, 6)) {
                    val hits = mutableListOf<TileHit>()
                    for (tile in tiles(bitmap, tileDiv, 0.25f)) {
                        val label = "$file|tileDiv=$tileDiv|tile=${tile.index}"
                        val hands = landmarker.detectFromBitmap(tile.bitmap)?.hands.orEmpty()
                        if (hands.isNotEmpty()) {
                            log("$label|STAGE1 hands=${hands.size}|tile=${tile.bitmap.width}x${tile.bitmap.height}")
                    ...
                    for ((h, hit) in hits.withIndex()) {
                        val crop = paddedCrop(bitmap, hit.tile, PAD_FRACTION)
                        ...
                        if (hands2.isNotEmpty()) {
                            ...
                            hands2.forEachIndexed { i, hand ->
                                log(
                                    "$file|stage2 hit=$h hand=$i|gesture=${HandGestureClassifier.classify(hand)?.gesture}|score=${
                                        rater.rate(
                                            hand,
                                        )?.score
    ...
    private fun tiles(bitmap: Bitmap, divisions: Int, overlapFraction: Float): List<Tile> {
        ...
        while (y < bitmap.height) {
            ...
            while (x < bitmap.width) {
                ...
                if (w > 0 && h > 0) {
                    result += Tile(Bitmap.createBitmap(bitmap, x, y, w, h), x, y)
                }
                x += step
            }
            y += step
    ...
    private fun paddedCrop(bitmap: Bitmap, tile: Tile, padFraction: Float): Tile {
