...
class LiteRtClipSearchDatasetTest {
        ...
        AndroidLiteRtAppClipSearchFactory(appContext)
            ...
            .use { clip ->
                for (name in samples.keys) {
                    ...
                    val similarity = cosineSimilarity(fullEmbedding, scaledEmbedding)
    ...
    private fun loadImageBytes(name: String): ByteArray = assets.open("search/$name.jpg").use { it.readBytes() }

    /**
     * Mirrors the app's scaled decode (sample down to [DECODE_MIN_DIM], then
     * JPEG-encode) so the equivalence check covers the production path.
     */
    private fun decodeScaledJpeg(bytes: ByteArray): ByteArray {
        ...
        return try {
            val w = bitmap.width
            val h = bitmap.height
            val sampleSize = (min(w, h) / DECODE_MIN_DIM).coerceAtLeast(1)
            val scaled = Bitmap.createScaledBitmap(bitmap, w / sampleSize, h / sampleSize, true)
    ...
    companion object {
        // Measured in Python on the same models: scaled (640) vs full decode
        // similarity is 0.9998; keep a generous margin for on-device decoders.
