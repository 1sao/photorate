...
/**
 * Full-image detection at very low confidence (0.1 / 0.05) for every sample.
 * The earlier sweep only went down to 0.25; if 4.jpg / 4_also respond at all,
 * the fix is simply a lower threshold (with the gesture classifier rejecting
 * the false positives). Landmarks are dumped so real hands can be told apart
 * from palm-detector hallucinations.
 */
    ...
    fun ultraLowConf() {
        for (conf in listOf(0.1f, 0.05f)) {
            ...
            val landmarker = factory.createFromOptions(
                HandLandmarkerOptions(maxNumHands = 4, minHandDetectionConfidence = conf, minHandPresenceConfidence = conf),
            )
            try {
                    ...
                    for (file in files) {
                        ...
                        if (hands.isNotEmpty()) {
                            ...
                            hands.forEachIndexed { i, hand ->
                                ...
                                log("conf=$conf $scoreDir/$file hand=$i gesture=$gesture score=$score points=$points")
                            }
                        } else {
