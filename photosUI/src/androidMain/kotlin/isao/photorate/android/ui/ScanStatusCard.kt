...
import isao.photorate.db.GalleryImageStatus.DONE
import isao.photorate.db.GalleryImageStatus.FAILED
import isao.photorate.db.GalleryImageStatus.PENDING
import isao.photorate.db.GalleryImageStatus.PROCESSING
...
/**
 * Scan status rendered as a full-width grid item (instead of a dialog): one
 * counter (label above animated count) per status. Pending, Processing and Done
 * share a single row of three; the Failed counter is hidden while there are no
 * errors and, once it appears, the four counters split into two rows of two.
 */
@Composable
fun ScanStatusCard(statusCounts: GalleryStatusCounts, modifier: Modifier = Modifier) {
    Surface(
        ...
        Column(Modifier.padding(20.dp).animateContentSize()) {
            ...
            val counters = listOf(PENDING, PROCESSING, DONE) +
                if (statusCounts[FAILED] > 0L) listOf(FAILED) else emptyList()
            if (counters.size == 3) {
                CounterRow(counters, statusCounts)
            } else {
                counters.chunked(2).forEach { rowStatuses ->
                    CounterRow(rowStatuses, statusCounts)
                }
            }
        }
    }
...
/**
 * Odometer-style count: each digit slides in/out when the displayed value
 * changes. Changes are processed sequentially — an incoming value never cancels
 * or overrides the in-flight slide, it is picked up once the slide finishes. A
 * change by one ticks at the default speed; a jump by more than one ticks at
 * double speed regardless of magnitude; when the value settles (no pending
 * change after a tick), the text plays a short bouncy settling spring.
 */
@Composable
    ...
    LaunchedEffect(Unit) {
        snapshotFlow { latest.value }.collect {
            ...
            while (true) {
                ...
                val tickMillis = if (abs(target - displayed) > 1L) {
                    FAST_TICK_MILLIS
                } else {
                    DEFAULT_TICK_MILLIS
                }
...
private fun AnimatedContentTransitionScope<Char>.defaultTick(): ContentTransform = if (targetState > initialState) {
...
/**
 * Emits a simulated stream of [GalleryStatusCounts] as if [totalPhotos] images
 * were being scanned one at a time. Runs forever, restarting the batch from
 * scratch once every photo is done/failed.
 */
...
@Composable
private fun rememberPreviewGalleryStatusCounts(totalPhotos: Long = 120L): State<GalleryStatusCounts> {
    val flow = remember(totalPhotos) { previewGalleryStatusFlow(totalPhotos) }
    return flow.collectAsState(
        initial = GalleryStatusCounts(
            counts = mapOf(
                PENDING to totalPhotos,
                PROCESSING to 0,
                DONE to 0,
