...
interface SearchDelegate : AutoCloseable {
    ...
    /**
     * Runs the CLIP search for the current query and returns the ranked result
     * uris; the Home screen applies them as a gallery grid filter. The full
     * ranked results also stay in [SearchUiState] for the results header and
     * leftover-match rendering.
     */
...
class DefaultSearchDelegate(
    ...
    // in-flight search (the CLIP session is single-use) instead of being dropped.
